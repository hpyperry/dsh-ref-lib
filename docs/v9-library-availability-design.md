...
## 14. 遗留备注：UI 反向数据同步（暂不处理）

**问题**：外部变更（目录被删/恢复）与 `/ref-lib` 命令（add/remove/note）修改**不会
自动反向同步到 Web UI**——面板与胶囊仅在挂载、打开面板、或本 UI 内操作后刷新
（`RefLibDock.refresh` 的现有触发点）。这与"命令修改后 UI 不刷新"是**同一类 UI 数据
同步问题**；v9 决策为**不做轮询/推送**（用户拍板，暂遗留，2026-08-22）。

当前影响面：外部删除目录后，注入侧**已实时生效**（每次模型请求探测，跳过失效库），
但 UI 侧需用户重新打开面板/刷新页面才看到红色提示。

后续方案候选（启动前需重新评估）：
1. 面板/胶囊可见期间轻量轮询（如 30s）；
2. node half 变更推送（webServer 事件 → client 刷新）；
3. 命令执行后由 node half 主动通知 client 刷新。
